...
def get_client_ip(request):
    # Try to get IP from headers (if behind proxy) or socket
...
async def handle_dashboard(request):
    ...
    active_nodes = sum(
        1 for n in NODES.values() if time.time() -
        n.get(
            "last_seen",
...
async def handle_settings_page(request):
    ...
    html = html.replace(
        "{web_title}",
        f"{_('web_settings_page_title', lang)} - Web Bot")
    ...
    html = html.replace("{val_timeout}", str(
    ...
    try:
        ...
        if traffic_interval < 5:
            return web.json_response(
                {"error": _("error_traffic_interval_low", lang)}, status=400)
        if traffic_interval > 100:
            return web.json_response(
                {"error": _("error_traffic_interval_high", lang)}, status=400)
...
async def handle_change_password(request):
    ...
    if user_id != ADMIN_USER_ID:
        return web.json_response(
            {"error": "Only Main Admin can change password"}, status=403)
    ...
    try:
        ...
        if not check_user_password(user_id, current_pass):
            lang = get_user_lang(user_id)
            return web.json_response(
                {"error": _("web_pass_wrong_current", lang)}, status=400)

        if not new_pass or len(new_pass) < 4:
            return web.json_response(
                {"error": "Password too short"}, status=400)
...
async def handle_user_action(request):
    ...
    try:
        ...
        if target_id == ADMIN_USER_ID:
            return web.json_response(
                {"error": "Cannot affect Main Admin"}, status=400)

        if action == 'delete':
            if target_id in ALLOWED_USERS:
                del ALLOWED_USERS[target_id]
                if str(target_id) in USER_NAMES:
                    del USER_NAMES[str(target_id)]
                if target_id in ALERTS_CONFIG:
                    del ALERTS_CONFIG[target_id]
                save_users()
                save_alerts_config()
                return web.json_response({"status": "ok"})
        elif action == 'add':
            ...
            save_users()
            return web.json_response(
                {"status": "ok", "name": USER_NAMES.get(str(target_id))})
    except Exception as e:
        return web.json_response({"error": str(e)}, status=500)
...
async def handle_reset_request(request):
    try:
        ...
        if user_id != ADMIN_USER_ID:
            ...
            return web.json_response(
                {"error": "not_found", "admin_url": admin_url}, status=404)
        ...
        if bot:
            try:
                kb = InlineKeyboardMarkup(inline_keyboard=[
                                          [InlineKeyboardButton(text="🔐 Сбросить пароль", url=reset_link)]])
...
async def handle_login_password(request):
    ...
    if not check_rate_limit(client_ip):
        return web.Response(
            text="Too many login attempts. Please wait 5 minutes.",
            status=429)
    ...
    if user_id != ADMIN_USER_ID:
        return web.Response(
            text="Password login available for Main Admin only.",
            status=403)
    ...
    if check_user_password(user_id, password):
        ...
        resp = web.HTTPFound('/')
        # FIX: Added samesite='Lax' against CSRF
        resp.set_cookie(
            COOKIE_NAME,
            session_token,
            max_age=604800,
...
async def handle_node_details(request):
    if not get_current_user(request):
        return web.json_response({"error": "Unauthorized"}, status=401)
    token = request.query.get("token")
    if not token or token not in NODES:
