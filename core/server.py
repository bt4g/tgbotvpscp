...
async def handle_login_password(request):
    ...
    if not check_rate_limit(client_ip):
        return web.Response(text="Too many login attempts. Please wait 5 minutes.", status=429)
    ...
    if check_user_password(user_id, password):
        ...
        resp.set_cookie(COOKIE_NAME, session_token, max_age=604800, httponly=True, samesite='Lax')
        return resp
...
async def handle_magic_login(request):
    ...
    resp.set_cookie(COOKIE_NAME, session_token, max_age=2592000, httponly=True, samesite='Lax')
    return resp
