...
async def api_get_notifications(request):
    ...
    # Фильтруем уведомления: оставляем только те, что включены у пользователя
    filtered = [n for n in list(shared_state.WEB_NOTIFICATIONS) if user_alerts.get(n['type'], False)]
    # Считаем непрочитанные персонально для пользователя
...
async def handle_dashboard(request):
    ...
    replacements = {
        ...
        "{role_badge}": role_badge_html, "{user_avatar}": _get_avatar_html(user),
        "{user_name}": user.get('first_name', 'User'), "{nodes_count}": str(nodes_count),
        "{active_nodes}": str(active_nodes), "{web_agent_stats_title}": _("web_agent_stats_title", lang).replace("Мониторинг (Агент)", "Сетевая активность").replace("Monitoring (Agent)", "Network Activity"),
        "{agent_ip}": AGENT_IP_CACHE, "{web_traffic_total}": _("web_traffic_total", lang),
        "{web_uptime}": _("web_uptime", lang), "{web_cpu}": _("web_cpu", lang),
        "{web_ram}": _("web_ram", lang), "{web_disk}": _("web_disk", lang),
        ...
        "{web_nodes_loading}": _("web_nodes_loading", lang), "{web_logs_btn_bot}": "Логи Бота" if lang == 'ru' else "Bot Logs",
        "{web_logs_btn_sys}": "Логи VPS" if lang == 'ru' else "VPS Logs",
        "{node_action_btn}": node_action_btn, "{settings_btn}": settings_btn,
        "{web_footer_powered}": _("web_footer_powered", lang), "{web_hint_cpu_usage}": _("web_hint_cpu_usage", lang),
        "{web_hint_ram_usage}": _("web_hint_ram_usage", lang), "{web_hint_disk_usage}": _("web_hint_disk_usage", lang),
        "{web_hint_traffic_in}": _("web_hint_traffic_in", lang), "{web_hint_traffic_out}": _("web_hint_traffic_out", lang),
        "{web_add_node_section}": _("web_add_node_section", lang), "{web_node_name_placeholder}": _("web_node_name_placeholder", lang),
        "{web_create_btn}": _("web_create_btn", lang), "{web_node_token}": _("web_node_token", lang),
        "{web_node_cmd}": _("web_node_cmd", lang), "{web_copied}": _("web_copied", lang),
    ...
    i18n_data = {
        "web_cpu": _("web_cpu", lang), "web_ram": _("web_ram", lang), 
        "web_no_nodes": _("web_no_nodes", lang), "web_loading": _("web_loading", lang),
        "web_error": _("web_error", lang, error=""), "web_conn_error": _("web_conn_error", lang, error=""),
        "web_log_empty": _("web_log_empty", lang), "web_access_denied": _("web_access_denied", lang),
        "web_copied": _("web_copied", lang), "web_no_notifications": _("web_no_notifications", lang),
        "web_clear_notifications": _("web_clear_notifications", lang), "modal_title_alert": _("modal_title_alert", lang),
        "modal_title_confirm": _("modal_title_confirm", lang), "web_clear_notif_confirm": _("web_clear_notifications", lang) + "?",
        "modal_btn_ok": _("modal_btn_ok", lang), "modal_btn_cancel": _("modal_btn_cancel", lang),
...
async def handle_nodes_list_json(request):
    ...
    for token, node in all_nodes.items():
        ...
        status = "offline"
        if node.get("is_restarting"): status = "restarting"
...
async def handle_settings_page(request):
    ...
    users_json, nodes_json = "null", "null"
    if is_admin:
    ...
    replacements = {
        "{web_title}": f"{_('web_settings_page_title', lang)} - Web Bot", "{user_name}": user.get('first_name'),
        "{user_avatar}": _get_avatar_html(user), "{users_data_json}": users_json,
        "{nodes_data_json}": nodes_json, "{keyboard_config_json}": json.dumps(KEYBOARD_CONFIG),
        "{val_cpu}": str(current_config.CPU_THRESHOLD), "{val_ram}": str(current_config.RAM_THRESHOLD),
        "{val_disk}": str(current_config.DISK_THRESHOLD), "{val_traffic}": str(current_config.TRAFFIC_INTERVAL),
        "{val_timeout}": str(current_config.NODE_OFFLINE_TIMEOUT), "{web_settings_page_title}": _("web_settings_page_title", lang),
        "{web_back}": _("web_back", lang), "{web_notif_section}": _("web_notif_section", lang),
        "{notifications_alert_name_res}": _("notifications_alert_name_res", lang), "{notifications_alert_name_logins}": _("notifications_alert_name_logins", lang),
        "{notifications_alert_name_bans}": _("notifications_alert_name_bans", lang), "{notifications_alert_name_downtime}": _("notifications_alert_name_downtime", lang),
        "{web_save_btn}": _("web_save_btn", lang), "{web_users_section}": _("web_users_section", lang),
        "{web_add_user_btn}": _("web_add_user_btn", lang), "{web_user_id}": _("web_user_id", lang),
        "{web_user_name}": _("web_user_name", lang), "{web_user_role}": _("web_user_role", lang),
        "{web_user_action}": _("web_user_action", lang), "{web_add_node_section}": _("web_add_node_section", lang),
        "{web_node_name_placeholder}": _("web_node_name_placeholder", lang), "{web_no_users}": _("web_no_users", lang),
        "{web_create_btn}": _("web_create_btn", lang), "{web_node_token}": _("web_node_token", lang),
        "{web_node_cmd}": _("web_node_cmd", lang), "{web_sys_settings_section}": _("web_sys_settings_section", lang),
        "{web_thresholds_title}": _("web_thresholds_title", lang), "{web_intervals_title}": _("web_intervals_title", lang),
        "{web_logs_mgmt_title}": _("web_logs_mgmt_title", lang), "{web_cpu_threshold}": _("web_cpu_threshold", lang),
        "{web_ram_threshold}": _("web_ram_threshold", lang), "{web_disk_threshold}": _("web_disk_threshold", lang),
        "{web_traffic_interval}": _("web_traffic_interval", lang), "{web_node_timeout}": _("web_node_timeout", lang),
        "{web_clear_logs_btn}": _("web_clear_logs_btn", lang), "{web_security_section}": _("web_security_section", lang),
        "{web_change_password_title}": _("web_change_password_title", lang), "{web_current_password}": _("web_current_password", lang),
        "{web_new_password}": _("web_new_password", lang), "{web_confirm_password}": _("web_confirm_password", lang),
        "{web_change_btn}": _("web_change_btn", lang), "{web_hint_cpu_threshold}": _("web_hint_cpu_threshold", lang),
        "{web_hint_ram_threshold}": _("web_hint_ram_threshold", lang), "{web_hint_disk_threshold}": _("web_hint_disk_threshold", lang),
        "{web_hint_traffic_interval}": _("web_hint_traffic_interval", lang), "{web_hint_node_timeout}": _("web_hint_node_timeout", lang),
        "{web_keyboard_title}": _("web_keyboard_title", lang), "{web_node_mgmt_title}": _("web_node_mgmt_title", lang),
        "{web_kb_desc}": _("web_kb_desc", lang), "{web_kb_btn_config}": _("web_kb_btn_config", lang),
        "{web_kb_enable_all}": _("web_kb_enable_all", lang), "{web_kb_disable_all}": _("web_kb_disable_all", lang),
        "{web_kb_modal_title}": _("web_kb_modal_title", lang), "{web_kb_done}": _("web_kb_done", lang),
        "{web_version}": CACHE_VER, "{web_update_section}": _("web_update_section", lang),
        "{web_update_placeholder}": _("web_update_placeholder", lang), "{web_update_check_btn}": _("web_update_check_btn", lang),
        "{web_update_do_btn}": _("web_update_do_btn", lang), "{web_notifications_title}": _("web_notifications_title", lang),
...
async def handle_clear_logs(request):
    ...
    try:
        ...
        dirs = []
        if target == 'bot': dirs = [BOT_LOG_DIR, WATCHDOG_LOG_DIR]
        elif target == 'node': dirs = [NODE_LOG_DIR]
        else: dirs = [BOT_LOG_DIR, WATCHDOG_LOG_DIR, NODE_LOG_DIR]
        for d in dirs:
            if os.path.exists(d):
...
async def handle_user_action(request):
    ...
    try:
        data = await request.json()
        act, tid = data.get('action'), int(data.get('id', 0))
        ...
        if act == 'delete':
            ...
            save_users(); return web.json_response({"status": "ok", "name": USER_NAMES.get(str(tid))})
    except Exception as e: return web.json_response({"error": str(e)}, status=500)
...
async def handle_set_language(request):
    ...
    try:
        data = await request.json()
        lang = data.get("lang")
        if lang in ["ru", "en"]: set_user_lang(user['id'], lang); return web.json_response({"status": "ok"})
...
async def handle_login_page(request):
    ...
    if BOT_USERNAME_CACHE is None:
        ...
        except Exception: BOT_USERNAME_CACHE = ""
    ...
    i18n_data = {"web_error": _("web_error", DEFAULT_LANGUAGE, error=""), "web_conn_error": _("web_conn_error", DEFAULT_LANGUAGE, error=""), "modal_title_alert": _("modal_title_alert", DEFAULT_LANGUAGE), "modal_title_confirm": _("modal_title_confirm", DEFAULT_LANGUAGE), "modal_title_prompt": _("modal_title_prompt", DEFAULT_LANGUAGE), "modal_btn_ok": _("modal_btn_ok", DEFAULT_LANGUAGE), "modal_btn_cancel": _("modal_btn_cancel", DEFAULT_LANGUAGE)}
...
async def handle_magic_login(request):
    token = request.query.get("token")
    if not token or token not in AUTH_TOKENS: return web.Response(text="Expired", status=403)
...
async def start_web_server(bot_instance: Bot):
    ...
    if ENABLE_WEB_UI:
        ...
        app.router.add_get('/', handle_dashboard); app.router.add_get('/settings', handle_settings_page)
        app.router.add_get('/login', handle_login_page); app.router.add_post('/api/login/request', handle_login_request)
        app.router.add_get('/api/login/magic', handle_magic_login); app.router.add_post('/api/login/password', handle_login_password)
        app.router.add_post('/api/login/reset', handle_reset_request); app.router.add_get('/reset_password', handle_reset_page_render)
        app.router.add_post('/api/reset/confirm', handle_reset_confirm); app.router.add_post('/api/auth/telegram', handle_telegram_auth)
        app.router.add_post('/logout', handle_logout); app.router.add_get('/api/node/details', handle_node_details)
        app.router.add_get('/api/agent/stats', handle_agent_stats); app.router.add_get('/api/nodes/list', handle_nodes_list_json)
        app.router.add_get('/api/logs', handle_get_logs); app.router.add_get('/api/logs/system', handle_get_sys_logs)
        app.router.add_post('/api/settings/save', handle_save_notifications); app.router.add_post('/api/settings/language', handle_set_language)
        app.router.add_post('/api/settings/system', handle_save_system_config); app.router.add_post('/api/settings/password', handle_change_password)
        app.router.add_post('/api/settings/keyboard', handle_save_keyboard_config); app.router.add_post('/api/logs/clear', handle_clear_logs)
        app.router.add_post('/api/users/action', handle_user_action); app.router.add_post('/api/nodes/add', handle_node_add)
        app.router.add_post('/api/nodes/delete', handle_node_delete)
        app.router.add_get('/api/update/check', api_check_update); app.router.add_post('/api/update/run', api_run_update)
        app.router.add_get('/api/notifications/list', api_get_notifications); app.router.add_post('/api/notifications/read', api_read_notifications)
    ...
    try: await site.start(); logging.info(f"Web Server started on {WEB_SERVER_HOST}:{WEB_SERVER_PORT}"); return runner
    except Exception as e: logging.error(f"Failed: {e}"); return None

async def agent_monitor():
    global AGENT_IP_CACHE, AGENT_FLAG
    import psutil; import requests
