...
async def handle_settings_page(request):
    ...
    html = html.replace("{web_title}", f"{_('web_settings_page_title', lang)} - Web Bot").replace("{user_name}", user.get('first_name')).replace("{user_avatar}", _get_avatar_html(user)).replace("{users_data_json}", users_json)
    for k in ["web_settings_page_title", "web_back", "web_notif_section", "notifications_alert_name_res", "notifications_alert_name_logins", "notifications_alert_name_bans", "notifications_alert_name_downtime", "web_save_btn", "web_users_section", "web_add_user_btn", "web_user_id", "web_user_name", "web_user_role", "web_user_action", "web_add_node_section", "web_node_name_placeholder", "web_create_btn", "web_node_token", "web_node_cmd", "web_sys_settings_section", "web_thresholds_title", "web_intervals_title", "web_logs_mgmt_title", "web_cpu_threshold", "web_ram_threshold", "web_disk_threshold", "web_traffic_interval", "web_node_timeout", "web_clear_logs_btn", "web_security_section", "web_change_password_title", "web_current_password", "web_new_password", "web_confirm_password", "web_change_btn"]:
        html = html.replace(f"{{{k}}}", _(k, lang))
    html = html.replace("{val_cpu}", str(current_config.CPU_THRESHOLD)).replace("{val_ram}", str(current_config.RAM_THRESHOLD)).replace("{val_disk}", str(current_config.DISK_THRESHOLD)).replace("{val_traffic}", str(current_config.TRAFFIC_INTERVAL)).replace("{val_timeout}", str(current_config.NODE_OFFLINE_TIMEOUT))
    for alert in ['resources', 'logins', 'bans', 'downtime']: html = html.replace(f"{{check_{alert}}}", "checked" if user_alerts.get(alert, False) else "")
    if user_id != ADMIN_USER_ID: html = html.replace('<div class="bg-white/60 dark:bg-white/5 backdrop-blur-md border border-white/40 dark:border-white/10 rounded-2xl p-6 mb-6 shadow-lg dark:shadow-none" id="securitySection">', '<div class="hidden">')
    i18n_data = {"web_saving_btn": _("web_saving_btn", lang), "web_saved_btn": _("web_saved_btn", lang), "web_save_btn": _("web_save_btn", lang), "web_change_btn": _("web_change_btn", lang), "web_error": _("web_error", lang, error=""), "web_conn_error": _("web_conn_error", lang, error=""), "web_confirm_delete_user": _("web_confirm_delete_user", lang), "web_no_users": _("web_no_users", lang), "web_clear_logs_confirm": _("web_clear_logs_confirm", lang), "web_logs_cleared": _("web_logs_cleared", lang), "error_traffic_interval_low": _("error_traffic_interval_low", lang), "error_traffic_interval_high": _("error_traffic_interval_high", lang), "web_logs_clearing": _("web_logs_clearing", lang), "web_logs_cleared_alert": _("web_logs_cleared_alert", lang), "web_pass_changed": _("web_pass_changed", lang), "web_pass_mismatch": _("web_pass_mismatch", lang)}
    html = html.replace("{i18n_json}", json.dumps(i18n_data))
    return web.Response(text=html, content_type='text/html')
...
async def handle_login_request(request):
    ...
    if bot:
        try:
            kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔓 Login", url=link)]])
...
async def handle_reset_request(request):
    try:
        ...
        if bot:
            try:
                kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔐 Reset", url=link)]])
                await bot.send_message(uid, "<b>🆘 Reset Password</b>", reply_markup=kb, parse_mode="HTML")
                return web.json_response({"status": "ok"})
            except: return web.json_response({"error": "bot_send_error"}, status=500)
...
async def handle_reset_page_render(request):
    ...
    html = load_template("login.html").replace("{error_block}", "").replace("{default_pass_alert}", "")
    return web.Response(text=html, content_type='text/html')
