...
async def send_support_message(bot: Bot, user_id: int, lang: str):
    try:
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=_("start_support_button", lang), url="https://yoomoney.ru/to/410011639584793")]
        ])
...
async def send_alert(bot: Bot, message_or_func: Union[str, Callable[[str], str]], alert_type: str, **kwargs):
    ...
    # 1. Сначала определяем, включен ли этот тип уведомлений хотя бы у кого-то
    users_to_alert = [uid for uid, cfg in ALERTS_CONFIG.items() if cfg.get(alert_type, False)]
    
    # Если получателей нет, полностью игнорируем алерт (не сохраняем и в WebUI)
    if not users_to_alert:
        return

    # 2. Сохраняем в WebUI только если есть активные получатели
    try:
        ...
        if web_text:
            ...
            # WEB_UNREAD_COUNT здесь больше не трогаем!
    except Exception as e:
        logging.error(f"Ошибка сохранения Web-уведомления: {e}")

    # 3. Отправка в Telegram пользователям, у которых этот тип ВКЛЮЧЕН
