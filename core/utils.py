                ...
                if response.status == 200:
                    ...
                    if data.get("status") == "success":
                        ...
                        if country_code and len(country_code) == 2:
                            return "".join(chr(ord(char.upper()) - 65 + 0x1F1E6) for char in country_code)
    except Exception as e:
...
def convert_json_to_vless(json_data, custom_name):
    try:
        ...
        if not outbounds: raise ValueError("No outbounds")
        outbound = next((ob for ob in outbounds if ob.get('protocol') == 'vless'), None)
        if not outbound: raise ValueError("No vless outbound")
        settings = outbound.get('settings')
        vnext = settings.get('vnext')
        if not vnext: raise ValueError("No vnext")
        server_info = vnext[0]
...
def format_traffic(bytes_value, lang: str):
    units = [get_text("unit_bytes", lang), get_text("unit_kb", lang), get_text("unit_mb", lang), get_text("unit_gb", lang), get_text("unit_tb", lang)]
    try:
    ...
    try:
        ...
        for line in output.split('\n'):
            ...
            if len(parts) >= 2:
                ...
                if 'amnezia' in image.lower() or name == 'amnezia-xray': return "amnezia", name
                if 'marzban' in image.lower() or 'marzban' in name: return "marzban", name
        return None, None
...
async def initial_restart_check(bot: Bot):
    if os.path.exists(RESTART_FLAG_FILE):
        try:
            with open(RESTART_FLAG_FILE, "r") as f: content = f.read().strip()
            if ':' in content:
        ...
        except Exception as e: logging.error(f"Restart check error: {e}")
        finally:
            if os.path.exists(RESTART_FLAG_FILE): os.remove(RESTART_FLAG_FILE)


async def initial_reboot_check(bot: Bot):
    if os.path.exists(REBOOT_FLAG_FILE):
        try:
            with open(REBOOT_FLAG_FILE, "r") as f: uid = int(f.read().strip())
            await bot.send_message(chat_id=uid, text=get_text("utils_server_rebooted", get_user_lang(uid)), parse_mode="HTML")
        except Exception as e: logging.error(f"Reboot check error: {e}")
        finally:
            if os.path.exists(REBOOT_FLAG_FILE): os.remove(REBOOT_FLAG_FILE)