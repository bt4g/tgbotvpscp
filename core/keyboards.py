...
def get_subcategory_keyboard(category_key: str,
                             user_id: int) -> ReplyKeyboardMarkup:
    ...
    is_admin = user_id == ADMIN_USER_ID or (
        ALLOWED_USERS.get(
            user_id,
            {}).get("group") == "admins" if isinstance(
            ALLOWED_USERS.get(user_id),
            dict) else ALLOWED_USERS.get(user_id) == "admins")
...
def get_keyboard_settings_inline(lang: str) -> InlineKeyboardMarkup:
    ...
    rows.append([InlineKeyboardButton(text=_("btn_back", lang),
                callback_data="close_kb_settings")])

    return InlineKeyboardMarkup(inline_keyboard=rows)


def get_manage_users_keyboard(lang: str):
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=_("btn_add_user", lang), callback_data="add_user"), InlineKeyboardButton(text=_("btn_delete_user", lang), callback_data="delete_user")], [InlineKeyboardButton(
        text=_("btn_change_group", lang), callback_data="change_group"), InlineKeyboardButton(text=_("btn_my_id", lang), callback_data="get_id_inline")], [InlineKeyboardButton(text=_("btn_back_to_menu", lang), callback_data="back_to_menu")]])
...
def get_group_selection_keyboard(lang: str, user_id_to_change=None):
    ...
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=_("btn_group_admins", lang), callback_data=f"set_group_{user_identifier}_admins"), InlineKeyboardButton(
        text=_("btn_group_users", lang), callback_data=f"set_group_{user_identifier}_users")], [InlineKeyboardButton(text=_("btn_cancel", lang), callback_data="back_to_manage_users")]])


def get_self_delete_confirmation_keyboard(user_id: int):
    lang = get_user_lang(user_id)
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=_("btn_confirm", lang), callback_data=f"confirm_self_delete_{
                                user_id}"), InlineKeyboardButton(text=_("btn_cancel", lang), callback_data="back_to_delete_users")]])


def get_reboot_confirmation_keyboard(user_id: int):
    lang = get_user_lang(user_id)
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=_(
        "btn_reboot_confirm", lang), callback_data="reboot"), InlineKeyboardButton(text=_("btn_reboot_cancel", lang), callback_data="back_to_menu")]])
...
def get_alerts_menu_keyboard(user_id: int):
    ...
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=_("alerts_menu_res", lang, status=(status_yes if res_enabled else status_no)), callback_data="toggle_alert_resources")], [InlineKeyboardButton(text=_("alerts_menu_logins", lang, status=(status_yes if logins_enabled else status_no)), callback_data="toggle_alert_logins")], [InlineKeyboardButton(
        text=_("alerts_menu_bans", lang, status=(status_yes if bans_enabled else status_no)), callback_data="toggle_alert_bans")], [InlineKeyboardButton(text=_("alerts_menu_downtime", lang, status=(status_yes if downtime_enabled else status_no)), callback_data="toggle_alert_downtime")], [InlineKeyboardButton(text=_("btn_back_to_menu", lang), callback_data="back_to_menu")]])


def get_nodes_list_keyboard(nodes_dict: dict,
...
def get_nodes_delete_keyboard(
        nodes_dict: dict, lang: str) -> InlineKeyboardMarkup:
    buttons = []
    for token, node_data in nodes_dict.items():
        name = node_data.get('name', 'Unknown')
        buttons.append([InlineKeyboardButton(
