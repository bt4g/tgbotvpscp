    ...
    def translate_button(btn: KeyboardButton) -> KeyboardButton:
        ...
        for key, text in default_strings.items():
            if text == btn.text:
                key_to_find = key
                break
        if key_to_find: return KeyboardButton(text=_(key_to_find, lang))
        return btn
    ...
    for row_template_keys in button_layout_keys:
        ...
        for btn_key in row_template_keys:
            ...
            if btn_text in available_buttons_texts:
                ...
                if button_obj: current_row.append(button_obj)
        if current_row: final_keyboard_rows.append(current_row)

    return ReplyKeyboardMarkup(keyboard=final_keyboard_rows, resize_keyboard=True, input_field_placeholder=_("main_menu_placeholder", lang))

def get_manage_users_keyboard(lang: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=_("btn_add_user", lang), callback_data="add_user"), InlineKeyboardButton(text=_("btn_delete_user", lang), callback_data="delete_user")],
        [InlineKeyboardButton(text=_("btn_change_group", lang), callback_data="change_group"), InlineKeyboardButton(text=_("btn_my_id", lang), callback_data="get_id_inline")],
...
def get_group_selection_keyboard(lang: str, user_id_to_change=None):
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=_("btn_group_admins", lang), callback_data=f"set_group_{user_identifier}_admins"), InlineKeyboardButton(text=_("btn_group_users", lang), callback_data=f"set_group_{user_identifier}_users")],
...
def get_self_delete_confirmation_keyboard(user_id: int):
    lang = get_user_lang(user_id)
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=_("btn_confirm", lang), callback_data=f"confirm_self_delete_{user_id}"), InlineKeyboardButton(text=_("btn_cancel", lang), callback_data="back_to_delete_users")]
    ])

def get_reboot_confirmation_keyboard(user_id: int):
    lang = get_user_lang(user_id)
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=_("btn_reboot_confirm", lang), callback_data="reboot"), InlineKeyboardButton(text=_("btn_reboot_cancel", lang), callback_data="back_to_menu")]
    ])
...
def get_alerts_menu_keyboard(user_id: int):
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=_("alerts_menu_res", lang, status=(status_yes if res_enabled else status_no)), callback_data="toggle_alert_resources")],
        [InlineKeyboardButton(text=_("alerts_menu_logins", lang, status=(status_yes if logins_enabled else status_no)), callback_data="toggle_alert_logins")],
        [InlineKeyboardButton(text=_("alerts_menu_bans", lang, status=(status_yes if bans_enabled else status_no)), callback_data="toggle_alert_bans")],
...
def get_node_management_keyboard(token: str, lang: str) -> InlineKeyboardMarkup:
    layout = [
        [InlineKeyboardButton(text=_("btn_selftest", lang), callback_data=f"node_cmd_{token}_selftest"), InlineKeyboardButton(text=_("btn_uptime", lang), callback_data=f"node_cmd_{token}_uptime")],
        [InlineKeyboardButton(text=_("btn_traffic", lang), callback_data=f"node_cmd_{token}_traffic"), InlineKeyboardButton(text=_("btn_top", lang), callback_data=f"node_cmd_{token}_top")],
        [InlineKeyboardButton(text=_("btn_speedtest", lang), callback_data=f"node_cmd_{token}_speedtest"), InlineKeyboardButton(text=_("btn_reboot", lang), callback_data=f"node_cmd_{token}_reboot")],
