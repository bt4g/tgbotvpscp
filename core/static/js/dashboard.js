...
function renderAgentChart(history) {
    ...
    if (agentChart) {
        ...
        agentChart = new Chart(ctx, { type: 'line', data: { labels, datasets: [{ label: 'RX', data: netRx, borderColor: '#22c55e', fill: true, backgroundColor: 'rgba(34,197,94,0.1)', tension: 0.3 }, { label: 'TX', data: netTx, borderColor: '#3b82f6', fill: true, backgroundColor: 'rgba(59,130,246,0.1)', tension: 0.3 }] }, options: opts });
    }
}
...
function renderCharts(history) {
    ...
    const commonOptions = {
        responsive: true, maintainAspectRatio: false, animation: false,
        interaction: { mode: 'index', intersect: false }, // ИСПРАВЛЕНО
        scales: { 
    ...
    if (chartRes) {
        ...
        chartRes = new Chart(document.getElementById('nodeResChart').getContext('2d'), { type: 'line', data: { labels, datasets: [{ label: 'CPU (%)', data: history.map(h => h.c), borderColor: '#3b82f6', pointRadius: 0 }, { label: 'RAM (%)', data: history.map(h => h.r), borderColor: '#a855f7', pointRadius: 0 }] }, options: { ...commonOptions, scales: { ...commonOptions.scales, y: { ...commonOptions.scales.y, max: 100 } } } });
    }
    ...
    if (chartNet) {
        ...
        chartNet = new Chart(document.getElementById('nodeNetChart').getContext('2d'), { type: 'line', data: { labels: labels.slice(1), datasets: [{ label: 'RX', data: netRxSpeed, borderColor: '#22c55e', fill: true, backgroundColor: 'rgba(34,197,94,0.1)', pointRadius: 0 }, { label: 'TX', data: netTxSpeed, borderColor: '#ef4444', pointRadius: 0 }] }, options: netOpts });
    }
}
