...
function checkSessionStatus() {
    ...
    fetch('/api/settings/language?_=' + Date.now(), {
            method: 'HEAD'
        })
        .then(res => {
...
async function clearNotifications(e) {
    ...
    try {
        ...
        if (res.ok) {
            updateNotifUI([], 0);
            if (window.showToast) window.showToast((typeof I18N !== 'undefined' && I18N.web_logs_cleared_alert) ? I18N.web_logs_cleared_alert : "Cleared!");
        }
    } catch (e) {
