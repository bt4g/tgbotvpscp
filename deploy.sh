...
verify_git_integrity() {
    echo -e "\n${C_BOLD}=== Проверка целостности ===${C_RESET}"
    
    if [ ! -d "${BOT_INSTALL_PATH}/.git" ]; then
        msg_error "Git-репозиторий не найден. Проверка невозможна."
        return 1
    fi

    cd "${BOT_INSTALL_PATH}" || return

    # 1. Проверка базы данных Git (поврежденные объекты)
    msg_info "1. Проверка внутренней базы данных Git..."
    if git fsck --full --no-dangling > /dev/null 2>&1; then
        msg_success "База данных Git в порядке (Corruption check passed)."
    else
        msg_error "КРИТИЧЕСКАЯ ОШИБКА: Репозиторий Git поврежден!"
        echo -e "${C_RED}Рекомендуется полная переустановка бота.${C_RESET}"
        return 1
    fi

    # 2. Проверка файлов на диске
    msg_info "2. Сверка файлов с эталоном (HEAD)..."
    # --porcelain дает вывод, удобный для скриптов
    CHANGED_FILES=$(git status --porcelain)

    if [ -z "$CHANGED_FILES" ]; then
        msg_success "Все файлы идентичны официальной версии (Tampering check passed)."
    else
        msg_warning "ВНИМАНИЕ: Обнаружены локальные изменения файлов:"
        echo -e "${C_YELLOW}--------------------------------------------------${C_RESET}"
        # Выводим список
        echo "$CHANGED_FILES"
        echo -e "${C_YELLOW}--------------------------------------------------${C_RESET}"
        echo -e "Легенда: M = Изменен, D = Удален, ?? = Лишний файл"
        msg_warning "Если вы не меняли код вручную, это может быть признаком повреждения или взлома."
    fi
}
...
main_menu() {
    ...
    while true; do
        ...
        echo -e "${C_CYAN}  9) Проверить целостность файлов${C_RESET}" 
        echo "  0) Выход"
        echo "--------------------------------------------------------"
        ...
        case $choice in
            ...
            9) verify_git_integrity; read -p "Нажмите Enter..." ;;
            0) break ;;
        esac
    done
}
