...
def extract_changelog_section(content: str, version: str) -> str:
    ...
    try:
        ...
        start_pattern = re.escape(f"## [{version}]")
...
def parse_version(v_str):
    """Простой парсер версий (1.14.0 -> tuple(1, 14, 0)) для сравнения."""
    try:
    ...
    try:
        ...
        if os.path.exists("CHANGELOG.md"):
            with open("CHANGELOG.md", "r", encoding="utf-8") as f:
                local_ver_str = get_version_from_content(f.read()) or "0.0.0"
        ...
        if target_branch and latest_ver_str != local_ver_str:
            ...
            await callback.message.edit_text(
                _("bot_update_available", lang, local=f"v{local_ver_str}", remote=f"v{latest_ver_str}", log=escape_html(changes_text)) + warning,
                reply_markup=keyboard,
                parse_mode="HTML"
            )
