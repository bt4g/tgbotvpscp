...
def compare_versions(ver1, ver2):
    ...
    Возвращает:
     1 если ver1 > ver2 (Remote > Local -> Обновление есть)
    -1 если ver1 < ver2
     0 если равны
    """
    def normalize(v):
        # Удаляем 'v' и разбиваем по точкам
        v = v.lower().lstrip('v')
...
async def get_remote_version_github(branch="main"):
    """Получает версию из README.md на GitHub (raw)"""
...
async def get_update_info():
    """
    Возвращает (local_ver, remote_ver, branch, update_available)
    """
    try:
        branch = await get_current_branch()
        branch = validate_branch_name(branch)

        # 1. Локальная версия (теперь берется из .env через utils)
        ...
        if remote_ver != "Unknown":
            # Если Remote > Local => Обновление есть
...
async def auto_update_checker(bot: Bot):
    ...
    while True:
        try:
            ...
            # Уведомляем только если есть обновление И мы о нем еще не говорили
            if available and remote_v != LAST_NOTIFIED_VERSION:
                ...
                warning = _(
                    ...
                    config.DEFAULT_LANGUAGE) if DEPLOY_MODE == "docker" else ""
        ...
        [
            ...
            InlineKeyboardButton(
                text=_(
                    "btn_update_system",
                    lang),
                callback_data="update_system_apt")
        ],
        [InlineKeyboardButton(text=_("btn_cancel", lang),
...
async def run_system_update(callback: types.CallbackQuery):
    ...
    if code == 0:
        ...
        text = _("update_fail", lang, code=code,
                 error=escape_html(err[-2000:]))
...
async def check_bot_update(callback: types.CallbackQuery):
    ...
    try:
        ...
        if available:
            ...
            warning = _(
                "bot_update_docker_warning",
                lang) if DEPLOY_MODE == "docker" else ""

            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(
                    text=_("btn_update_bot_now", lang), callback_data=f"do_bot_update:{branch}")],
                [InlineKeyboardButton(
                    text=_("btn_cancel", lang), callback_data="back_to_menu")]
            ])

            await callback.message.edit_text(
                _("bot_update_available", lang, local=local_v,
                  remote=remote_v, log=escape_html(changes_log)) + warning,
                reply_markup=keyboard,
                parse_mode="HTML"
            )

        else:
            # Обновления нет (Локальная >= Remote)
            await callback.message.edit_text(
...
async def run_bot_update(callback: types.CallbackQuery):
    ...
    try:
        ...
        # Это сообщение может не успеть отправиться, если бот перезагрузится быстро
        # Но логика старта (core.utils.initial_restart_check) обработает это после запуска
