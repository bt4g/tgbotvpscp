...
async def cq_node_select(callback: types.CallbackQuery):
    ...
    if is_restarting:
        await callback.answer(
            _("node_restarting_alert", lang, name=node.get("name")), show_alert=True
        )
        return
...
async def process_node_name(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        _("node_add_success_token", lang, name=name, token=token), parse_mode="HTML"
    )
    await state.clear()
...
async def cq_node_command(callback: types.CallbackQuery):
    ...
    await callback.answer(
        _("node_cmd_sent", lang, cmd=cmd_name, name=node.get("name")), show_alert=False
    )
    ...
    while True:
        try:
            ...
            for token, node in nodes.items():
                ...
                is_dead = (now - last_seen >= config.NODE_OFFLINE_TIMEOUT) and (
                    last_seen > 0
                )
