...
async def cq_node_command(callback: types.CallbackQuery):
    ...
    if cmd == "reboot":
        ...
        logging.info(f"User {user_id} requested reboot for node {node.get('name')}")
                    ...
                    async def check(metric, current, threshold, key_high, key_norm):
                        ...
                        if current >= threshold:
                            if not state["active"] or (now - state["last_time"] > config.RESOURCE_ALERT_COOLDOWN):
                                ...
                                state["last_time"] = now
                                updated = True
                                logging.warning(f"Node {name} high {metric}: {current}%") # Log high resource
                            ...
                            state["last_time"] = 0
                            updated = True
                            logging.info(f"Node {name} {metric} normalized.") # Log normalized
