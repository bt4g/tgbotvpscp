...
def start_background_tasks(bot: Bot) -> list[asyncio.Task]:
    ...
    ssh_log = None
    if os.path.exists(get_host_path("/var/log/secure")): ssh_log = get_host_path("/var/log/secure")
    elif os.path.exists(get_host_path("/var/log/auth.log")): ssh_log = get_host_path("/var/log/auth.log")
    
    if ssh_log:
        tasks.append(asyncio.create_task(reliable_tail_log_monitor(bot, ssh_log, "logins", parse_ssh_log_line), name="LoginsMonitor"))
    
    tasks.append(asyncio.create_task(reliable_tail_log_monitor(bot, get_host_path("/var/log/fail2ban.log"), "bans", parse_f2b_log_line), name="BansMonitor"))
    return tasks
...
async def cq_toggle_alert(callback: types.CallbackQuery):
    ...
    map_name = {"resources": "notifications_alert_name_res", "logins": "notifications_alert_name_logins", "bans": "notifications_alert_name_bans", "downtime": "notifications_alert_name_downtime"}
    name = _(map_name.get(alert_type, "error"), lang)
    status = _("notifications_status_on", lang) if new_state else _("notifications_status_off", lang)
...
async def parse_ssh_log_line(line: str) -> dict | None:
    ...
    if match:
        try:
            ...
            return {"key": "alert_ssh_login_detected", "params": {"user": user, "flag": flag, "ip": ip, "time": datetime.now().strftime('%H:%M:%S'), "tz": get_server_timezone_label()}}
        except: return None
    return None
...
async def parse_f2b_log_line(line: str) -> dict | None:
    ...
    if match:
        try:
            ...
            # ИСПРАВЛЕНО: прямой await
            flag = await get_country_flag(ip)
            return {"key": "alert_f2b_ban_detected", "params": {"flag": flag, "ip": ip, "time": datetime.now().strftime('%H:%M:%S'), "tz": get_server_timezone_label()}}
        except: return None
    return None
...
async def resource_monitor(bot: Bot):
    ...
    while True:
        try:
            ...
            def check(metric, val, thresh, key_high, key_rep, key_norm):
                if val >= thresh:
                    if not RESOURCE_ALERT_STATE[metric]:
                        alerts.append((key_high, {"usage": val, "threshold": thresh}))
                        ...
                        alerts.append((key_rep, {"usage": val, "threshold": thresh}))
                        LAST_RESOURCE_ALERT_TIME[metric] = now
            ...
            check("cpu", cpu, config.CPU_THRESHOLD, "alert_cpu_high", "alert_cpu_high_repeat", "alert_cpu_normal")
            check("ram", ram, config.RAM_THRESHOLD, "alert_ram_high", "alert_ram_high_repeat", "alert_ram_normal")
            check("disk", disk, config.DISK_THRESHOLD, "alert_disk_high", "alert_disk_high_repeat", "alert_disk_normal")

            if alerts:
                await send_alert(bot, lambda lang: "\n\n".join([_(k, lang, **p) for k, p in alerts]), "resources")
        except Exception as e: logging.error(f"ResMonitor error: {e}")
    ...
    while True:
        ...
        try:
            ...
            async for line in proc.stdout:
                ...
                if l:
                    data = await parser(l)
                    if data: await send_alert(bot, lambda lang: _(data["key"], lang, **data["params"]), alert_type)
