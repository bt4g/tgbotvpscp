# 🧩 Инструкция по добавлению модуля

Проект построен на модульной архитектуре. Каждый функциональный блок (например, `uptime`, `speedtest`) — это отдельный Python-файл в папке `modules/`. Чтобы добавить новую функцию, нужно выполнить 4 шага.

### 📂 Шаг 1: Создание файла модуля

Создайте новый файл в директории `modules/`. Например, назовем его `my_feature.py`.

**Путь:** `/opt/tg-bot/modules/my_feature.py`

Вставьте в него следующий шаблон кода. Это минимально необходимая структура для работы с ядром бота.

```python
import asyncio
import logging
from aiogram import Dispatcher, types
from aiogram.types import KeyboardButton

# Импорты ядра
from core.i18n import _, I18nFilter, get_user_lang
from core import config
from core.auth import is_allowed, send_access_denied_message
from core.messaging import delete_previous_message
from core.shared_state import LAST_MESSAGE_IDS

# 1. Уникальный ключ кнопки (используется в i18n)
BUTTON_KEY = "btn_my_feature"

# 2. Функция, возвращающая кнопку для меню
def get_button() -> KeyboardButton:
    # Возвращает объект кнопки с текстом на языке по умолчанию
    return KeyboardButton(text=_(BUTTON_KEY, config.DEFAULT_LANGUAGE))

# 3. Регистрация обработчиков (Handlers)
def register_handlers(dp: Dispatcher):
    # Фильтр I18nFilter позволяет реагировать на текст кнопки на любом языке
    dp.message(I18nFilter(BUTTON_KEY))(my_feature_handler)

# 4. Основная логика модуля
async def my_feature_handler(message: types.Message):
    user_id = message.from_user.id
    chat_id = message.chat.id
    lang = get_user_lang(user_id) # Получаем язык пользователя
    command_name = "my_feature"   # Имя для проверки прав и удаления старых сообщений

    # --- Проверка прав ---
    if not is_allowed(user_id, command_name):
        await send_access_denied_message(message.bot, user_id, chat_id, command_name)
        return

    # --- Очистка чата ---
    # Удаляет предыдущее сообщение от этой команды, чтобы не засорять чат
    await delete_previous_message(user_id, command_name, chat_id, message.bot)

    # --- Показываем статус "печатает..." ---
    await message.bot.send_chat_action(chat_id=chat_id, action="typing")

    # --- Ваша логика ---
    # Пример: выполнение какой-то операции
    result_data = "Работа выполнена успешно!" 
    
    # Формируем ответ (текст берем из i18n)
    response_text = _("my_feature_response", lang, data=result_data)

    # --- Отправка ответа ---
    sent_message = await message.answer(response_text, parse_mode="HTML")
    
    # Сохраняем ID сообщения, чтобы удалить его при следующем вызове
    LAST_MESSAGE_IDS.setdefault(user_id, {})[command_name] = sent_message.message_id
```

-----

### 🌐 Шаг 2: Добавление переводов (i18n)

Чтобы текст кнопок и сообщений менялся в зависимости от языка пользователя, нужно добавить ключи в словарь переводов.

**Файл:** `/opt/tg-bot/core/i18n.py`

Найдите словарь `STRINGS` и добавьте в него ваши ключи (`btn_my_feature`, `my_feature_response` и т.д.) для русского (`ru`) и английского (`en`) языков.

```python
STRINGS = {
    'ru': {
        # ... существующие ключи ...
        "btn_my_feature": "✨ Моя Функция",
        "my_feature_response": "✅ Результат работы функции:\n<b>{data}</b>",
    },
    'en': {
        # ... existing keys ...
        "btn_my_feature": "✨ My Feature",
        "my_feature_response": "✅ Feature result:\n<b>{data}</b>",
    }
}
```

-----

### ⚙️ Шаг 3: Регистрация модуля в боте

Теперь нужно сообщить главному файлу бота о существовании нового модуля.

**Файл:** `/opt/tg-bot/bot.py`

1.  **Импортируйте модуль** в начале файла:

    ```python
    from modules import (
        selftest, traffic, uptime, ..., 
        my_feature  # <-- Добавьте ваш модуль сюда
    )
    ```

2.  **Включите флаг** (опционально, для удобства управления):

    ```python
    ENABLE_MY_FEATURE = True
    ```

3.  **Зарегистрируйте модуль** в функции `load_modules()`:

    ```python
    def load_modules():
        # ... другие модули ...
        
        if ENABLE_MY_FEATURE:
            # Обычная регистрация (доступно всем пользователям из whitelist)
            register_module(my_feature) 
            
            # ИЛИ только для админов:
            # register_module(my_feature, admin_only=True)
            
            # ИЛИ только для Root (полный доступ к системе):
            # register_module(my_feature, root_only=True)
    ```

-----

### 🔄 Шаг 4: Перезапуск

Чтобы изменения вступили в силу, перезапустите бота.

**Если используете Systemd:**

```bash
sudo systemctl restart tg-bot
```

**Если используете Docker:**

```bash
docker compose restart bot-secure
# или
docker compose restart bot-root
```

### ✅ Готово\!

Теперь в главном меню бота появится кнопка **"✨ Моя Функция"**. При нажатии на неё выполнится код из вашего файла `my_feature.py`.