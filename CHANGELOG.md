...
* **SSE (Real-time):** Полный переход на технологию **Server-Sent Events**. Данные дашборда, логи и статусы нод теперь обновляются в реальном времени без задержек и лишних запросов.
* **Редактирование Нод:** Реализована возможность **переименования нод** прямо из веб-интерфейса.
* **Интерактивные Логи:**
* Мгновенный поток логов через SSE.
* Умный авто-скролл (останавливается при просмотре истории).
* Визуальные индикаторы загрузки (спиннеры).

### 🛡️ Безопасность:

* **Шифрование Конфигурации:** Внедрено **Fernet-шифрование** (AES) для файлов конфигурации на диске. Теперь чувствительные данные (`nodes.json`, `users.json`) защищены от прямого чтения.
* **Web Data Obfuscation:** IP-адреса и токены теперь передаются в веб-интерфейс в зашифрованном виде, предотвращая их утечку через инструменты разработчика в браузере.

### ✨ Улучшено:

* **UI/UX:** Добавлены эффекты размытия (Blur) для скрытия контента (например, логов) от пользователей без прав доступа.
* **Производительность:** Снижена нагрузка на сеть и процессор за счет отказа от постоянного пулинга (polling) API.
