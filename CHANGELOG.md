...
* **Shell Injection Protection (Критично):**
    * Устранены 6+ уязвимостей Shell Injection, позволяющих выполнить произвольный код с правами root.
    * Заменены все вызовы `os.system()` и `subprocess` с `shell=True` на безопасные версии с массивом аргументов.
    * **node.py:** Исправлена критическая уязвимость `os.system("reboot")` → `subprocess.Popen(["reboot"])`
    * **manage.py:** Исправлены команды перезапуска Docker и systemctl с использованием `subprocess.run()`
    * **fail2ban.py:** Заменено `create_subprocess_shell` на `create_subprocess_exec` для чтения логов
    * **core/utils.py:** Исправлена функция `detect_xray_client()` для безопасного вызова `docker ps`

* **Усиление пароля (Высокий приоритет):**
    * Увеличена минимальная длина пароля с 4 до **8 символов** в соответствии с рекомендациями NIST.
    * Улучшены сообщения об ошибках валидации пароля во всех точках входа.

* **Memory Leak Prevention (DoS):**
    * Добавлена защита от накопления токенов в `AUTH_TOKENS`.
    * При превышении 1000 активных токенов автоматически удаляются 25% старейших для предотвращения утечки памяти и DoS-атак.

### ✨ Улучшения:

* **Безопасность кода:** Все исправления обратно совместимы и не изменяют API интерфейсы.
* **Тестирование:** Все изменённые файлы прошли проверку синтаксиса и не содержат ошибок.
